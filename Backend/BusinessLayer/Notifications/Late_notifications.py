...
class LateNotifications:
    ...
    def add_notification(self, user_id, sender_id, message, receiver_id,need_approval=False):
        notification_id = self.generateNotificationId()
        if user_id not in self.notifications:
            self.notifications[user_id] = []
        self.notifications[user_id].append(Notification.create(sender_user_id=sender_id,notification_id=notification_id, receiver_user_id=receiver_id, message=message, timestamp=datetime.now(),  need_approval=need_approval))
